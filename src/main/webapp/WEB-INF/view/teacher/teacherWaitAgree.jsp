...
<script>
    layui.use(['table','layer','jquery'], function(){
        ...
        table.render({
            ...
            ,height: 480
            ,width: 1080
            ,url: "teacherGetWaitAgreeLeaveMsg?gradename=" + gradename //数据接口
            ,page: true //开启分页
            ,limit:8//每页显示几条数据
        ...
        table.on('toolbar(test)', function(obj){
            switch(obj.event){
                ...
                case 'agree':
                    ...
                    if (data.length < 1){
                        layer.msg("请选择要同意的请假申请")
                    }else {
                        layer.confirm('确定要同意吗','同意请假申请',function () {
                            //获取请假信息编号
                            var leaveids = "";
                            ...
                            $.ajax({
                                ...
                                ,success:function (data) {
                                    ...
                                    table.reload("demo",function () {
                                        url:"teacherGetWaitAgreeLeaveMsg?gradename=" + gradename
                                    })
                                }
                                ,error:function () {
                                    layer.msg("执行失败");
                ...
                case 'refuse':
                    ...
                    var data = checkStatus.data;
                    if (data.length < 1){
                        layer.msg("请选择要拒绝的请假申请")
                    }else {
                        layer.confirm('确定要拒绝吗','拒绝请假申请',function () {
                            //获取请假信息编号
                            var leaveids = "";
                            for (var i = 0; i < data.length; i++) {
                                leaveids += data[i].leaveid + ",";
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'agree'){
                layer.confirm('是否同意该条请假申请', function(index){
                    layer.close(index);
                    //向服务端发送删除指令
                    ...
                    $.ajax({
                        ...
                        , success: function (data) {
                            ...
                            if ("true" === data) {
                                layer.msg("同意成功",function() {
                                    setTimeout('window.location.reload()', 10);
                                })
                            } else {
                                layer.msg("同意失败",function() {
                                    setTimeout('window.location.reload()', 10);
                                });
                            }
                        }
                ...
                layer.confirm('是否拒绝该条请假申请', function(index){
                    layer.close(index);
                    //向服务端发送删除指令
                    var leaveid = data.leaveid;
                    $.ajax({
                ...
                layer.open({
                    ...
                    , content:"teacherViewLeaveMsg?leaveid=" +leaveid
                    , area: ["625px","500px"]
                    , end: function () {
                        //刷新当前页
                        $(".layui-laypage-btn").click();
